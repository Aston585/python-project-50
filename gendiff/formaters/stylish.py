...
def preparation_of_values(data):
    ...
    for item in data:
        ...
        if item.get('status') == 'changing':
            item = [{'status': 'removed', 'key': item.get('key'), 'value': get_nested_object(item.get('from'))},{'status': 'added', 'key': item.get('key'), 'value': get_nested_object(item.get('to'))}]
            row.extend(item)
            continue
        row.append(item)
    return row
...
def get_nested_object(data):
    ...
    for key, value in data.items():
        result.append({'status': 'unchanging', 'key': key, 'value': get_nested_object(value)})
    return result
